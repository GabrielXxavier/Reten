...
export async function DashboardCards() {
  ...
  const alunosAltoRisco = alunos.filter((aluno) => aluno.Prob_Churn >= 0.30).length
  const alunosMedioRisco = alunos.filter((aluno) => aluno.Prob_Churn >= 0.20 && aluno.Prob_Churn < 0.30).length
  const alunosBaixoRisco = alunos.filter((aluno) => aluno.Prob_Churn < 0.2).length
    ...
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">Probabilidade &gt; 30%</p>
        </CardContent>
      </Card>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">Probabilidade entre 20-30%</p>
        </CardContent>
      </Card>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">Probabilidade &lt; 20%</p>
        </CardContent>
      </Card>
    </div>
  )
